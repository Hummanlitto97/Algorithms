...
namespace LAB1.InsertionSort
{
    class MasyvasFailas : DuomenuMasyvas 
    {
        ...
        {
            Byte[] duomenys = new Byte[4];
            BitConverter.GetBytes(reiksmePraeito).CopyTo(duomenys, 0);
            BitConverter.GetBytes(dabartineReiksme).CopyTo(duomenys, 1);
            rs.Seek(indeksas - 1, SeekOrigin.Begin);
            rs.Write(duomenys, 0, 2);
            return true;
        }
    }
}
